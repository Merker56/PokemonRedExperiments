...
class RedGymEnv(Env):
    ...
    def step(self, action):
        ...
        #self.stream_metadata['extra'] = self.get_lead_pokemon_info()
    ...
    def get_levels_reward(self): 
        ...
        level_sum = sum(self.pokemon_levels.values()) - 4
        if level_sum < 20:
            scaled = level_sum 
        else:
    ...
    def get_pokedex_caught(self):
        ...
        return max(self.num_caught - 1, 0)
    ...
    def get_pokedex_seen(self):
        ...
        return max(sum([self.bit_count(n) for n in num_seen]) - 3, 0)
    ...
    def get_game_state_reward(self, print_stats=True):
        ...
        state_scores = {
            ...
            "level": self.reward_scale * self.get_levels_reward() * 0.5,
            "heal": self.reward_scale * self.total_healing_rew * 3,
            ...
            "explore": self.reward_scale * self.explore_weight * len(self.seen_coords),
            "items": self.reward_scale * self.get_item_reward() * 0.05,
            "seen": self.reward_scale * self.get_pokedex_seen(),
            "captured": self.reward_scale * self.get_pokedex_caught(),
            #"PP": self.reward_scale * self.total_healing_pp_rew * 0.05,
        }
